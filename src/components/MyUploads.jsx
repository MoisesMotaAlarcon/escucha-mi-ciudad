...
function MyUploads() {
      ...
      async () => {
        try {
          ...
          const docRef = await addDoc(collection(db, "uploads"), {
            ...
            createdAtMs,                  // client time para ordenar ya
          });

          // Optimistic update (el snapshot reemplazará el estado enseguida)
          const newUpload = {
            id: docRef.id,
            imageUrl: downloadURL,
            filePath,
  ...
  // Escucha en tiempo real (sin orderBy; ordenamos en memoria)
  useEffect(() => {
    if (!user) return;
      ...
      (snap) => {
        const data = snap.docs.map((d) => {
          ...
          return {
            ...
            userId: v.userId,
            createdAt: v.createdAt,   // Timestamp server (puede tardar)
            createdAtMs: v.createdAtMs || 0, // número (inmediato)
          };
        });
  ...
  const handleDelete = async (item) => {
    ...
    try {
      ...
      // No tocamos estado: onSnapshot lo actualizará
    } catch (err) {

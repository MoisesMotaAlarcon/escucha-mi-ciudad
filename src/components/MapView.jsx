...
// Centro aproximado de la Península Ibérica (España)
const SPAIN_CENTER = [40.0, -3.7];
const SPAIN_ZOOM = 6;

// Radio en metros para buscar lugares en Overpass (ajusta si quieres)
const SEARCH_RADIUS_M = 1500;

function MapView() {
  const mapRef = useRef(null);            // instancia Leaflet
  const userMarkerRef = useRef(null);     // marcador de usuario
  const placesLayerRef = useRef(null);    // capa con monumentos/lugares
  ...
  useEffect(() => {
    ...
    // Capa para marcadores de lugares (se limpia cada búsqueda)
  ...
  /* ------------------------------------------------------------------
     Geolocaliza al usuario y busca lugares cercanos
  ------------------------------------------------------------------ */
  const geolocateAndLoad = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        // Se queda en España; no mostramos alerta para no molestar
      },
  ...
  /* ------------------------------------------------------------------
     ...
     Incluimos categorías: monumentos, ayuntamientos, edificios públicos,
  ...
  const fetchNearbyPlaces = async (latitude, longitude) => {
    ...
    const q = `
      ...
      // Ayuntamientos, edificios públicos y plazas
    ...
    try {
      ...
      data.elements.forEach((el) => {
        ...
        // Cuando se abra este popup, conectar botón -> navegación
        marker.on("popupopen", () => {
